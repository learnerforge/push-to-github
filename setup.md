...
URGithub's **core engine, GUI, wizard, and file watcher are cross-platform** — only the platform table below depends on **operating-system-specific integrations**.
...
> On **Linux / macOS** there is no Task Scheduler — run
> `python3 urgithub.py --run scheduled` from your own `cron` / `systemd` /
> `launchd` job (see [Part G](#part-g--scheduled-tasks-per-os)).
